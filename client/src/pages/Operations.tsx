    ...
    <div className="space-y-5">
      ...
      <section className="rounded-md border border-slate-200 bg-white p-4">
        <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-7">
          ...
          <select value={filters.categoryGroup || ''} onChange={(event) => updateFilter('categoryGroup', event.target.value)} className={input}><option value="">All categories</option>{['Outdoor', 'Auto', 'Bus', 'Mobile Van', 'A3 Screens'].map((item) => <option key={item}>{item}</option>)}</select>
